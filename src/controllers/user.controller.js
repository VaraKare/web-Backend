...
const getUserChannelProfile = asyncHandler (async (req,res) => {
    const { username } = req.params // to get to channel profile we got to its url so params
    ...
    const channel = await User.aggregate([
        ...
        {
            $lookup: {
                from: "subscriptions", //everyting in models gets plurl and lowercase 
                localField: "_id",
                foreignField: "channel", // we will get subscribers by counting channels
                as: "subscribers"  
            }
        },
        {
            $lookup: {
                from: "subscriptions", //everyting in models gets plurl and lowercase 
                localField: "_id",
                foreignField: "subscriber", // we will get channels by scbscribers data
                as: "subscribedTo"  
            }
        },
        {
            $addFields : {
                subscribersCount : {
                    $size : "$subscribers" //used "$" since its a field now
                },
                channelsSubscribedToCount : {
                    $size : "$subscribedTo" //used "$" since its a field now
                },
                isSubscribed : {
                    $cond: {
                        if: {$in : [req.user?._id, "$subscribers.subscriber"]}, //in user data fetched from certain id , we are going to certain field subscribers and going to subscriber to check true or false
                        then: true,
                        else:false,
                    }
                }
        ...
        {
            $project : {
                fullName : 1, //1 represents true to project to user/frontend
                username : 1,
                subscribersCount : 1,
